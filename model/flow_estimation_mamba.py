...
from .warplayer import backward_warp as warp
from .refine_mamba import *
...
class MultiScaleFlow_mamba(nn.Module):
    def __init__(self, backbone, **kargs):
        super(MultiScaleFlow_mamba, self).__init__()
        print('MultiScaleFlow_mamba')
    ...
    def calculate_flow(self, imgs, timestep, local=True, af=None):
        img0, img1 = imgs[:, :3], imgs[:, 3:6]
        B = img0.size(0)
        flow, mask = None, None
        if af is None:
            af = self.feature_bone(img0, img1)
        timestep=timestep.to(device=imgs[:, :1].device)
    ...
    def calculate_flow_hr(self, imgs, timestep, local=True, af=None):
        img0, img1 = imgs[:, :3], imgs[:, 3:6]
        B = img0.size(0)
        flow, mask = None, None
        if af is None:
    ...
    def forward(self, x , gt=None, timestep=0.5, local=True, scale=0):
        if scale > 0: 
            x_o = x
        ...
        flow = None
        af = self.feature_bone(img0, img1)
        #timestep=timestep.to(device=x[:, :1].device)
        timestep = ((x[:, :1].clone() * 0 + 1) * (timestep.float() if type(timestep) is not float else timestep)).to(device=x[:, :1].device)
        for i in range(self.flow_num_stage):
            if flow != None:
